            ...
            <div class="detail-card">
                ...
                <div class="detail-card-body">
                    <table class="detail-table table-borderless">
                        ...
                        <tr>
                            <th><i class="bi bi-calendar-check"></i> Tanggal Mulai</th>
                            <td>{{ $indent->tanggal_mulai ?? '-' }}</td>
                        </tr>
                        <tr>
                            <th><i class="bi bi-calendar-x"></i> Tanggal Selesai</th>
                            <td>{{ $indent->tanggal_selesai ?? '-' }}</td>
                        </tr>
                        <tr>
                            <th><i class="bi bi-file-earmark-text-fill"></i> Berkas</th>
                            <td>
                                @if ($indent->berkas_indent && $indent->berkas_indent->isNotEmpty())
                                    @foreach ($indent->berkas_indent as $berkas)
                                        @php
                                            $path = str_replace('public/', '', $berkas->path);
                                        @endphp
                    ...
                    <div class="action-buttons">
                        <a href="{{ route('admin.tabelindent') }}" class="btn btn-secondary btn-action">
                            <i class="bi bi-arrow-left"></i> Kembali
                        </a>
                        @if($indent->status == 'menunggu')
                            <div>
                                <button type="button" class="btn btn-success btn-action"
                                    onclick="confirmTerima({{ $indent->id }}, '{{ $indent->nama }}', '{{ $indent->asal_instansi }}')">
                                    <i class="bi bi-check-circle"></i> Terima
                                </button>

                                <button type="button" class="btn btn-danger btn-action"
                                    onclick="confirmTolak({{ $indent->id }}, '{{ $indent->nama }}', '{{ $indent->asal_instansi }}')">
                                    <i class="bi bi-x-circle"></i> Tolak
                                </button>
                            </div>
                        @else
                            <div class="d-flex align-items-center gap-3">
                                @if($indent->status == 'diterima' || $indent->status == 'ditolak')
                                    <a href="{{ route('admin.cetakSuratBalasanIndent', $indent->id) }}" 
                                       target="_blank"
                                       class="btn btn-primary btn-action">
                                        <i class="bi bi-printer-fill"></i> Cetak Surat Balasan
                                    </a>
                                @endif
                                <div class="alert {{ $indent->status == 'diterima' ? 'alert-success' : 'alert-danger' }} mb-0">
                                    <i class="bi {{ $indent->status == 'diterima' ? 'bi-check-circle' : 'bi-x-circle' }}"></i>
                                    Pendaftar ini sudah <strong>{{ $indent->status == 'diterima' ? 'diterima' : 'ditolak' }}</strong>
                                </div>
                            </div>
                        @endif
                    </div>
